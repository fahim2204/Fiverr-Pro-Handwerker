            ...
            <header className="hero-img">
                ...
                <div className="container-fluid mt-5 pt-5">
                    <div className="container">
                        <h1 className="text-center display-3 fw-bold text-white">Garten und Landschaftsbauer -
                            Stadt wählen</h1>
                                    ...
                                    <div className="row py-3">
                                        {
                                            CityList.filter(item => item.city.startsWith(letter)).map((item) => {
                                                return (
                                                    <>
                                                        <div className="city-box col-6 col-sm-4 col-md-3"><Link className="m-3 w-100 border-bottom" href={`/garten-und-landschaftsbauer/${item.city}`}>{item.city.toLocaleUpperCase()}</Link></div>
                                                    </>
                                                )
                                            })
                                        }
                ...
                <div className="row py-3">
                    {
                        CityList.filter(item => item.city.charAt(0).match(/[^a-z]/i)).map((item) => {
                            return (
                                <>
                                    <div className="col-3 mb-3 hov-cl-yellow d-flex justify-content-center align-items-center"><Link className="m-3 w-100 border-bottom" href={`/garten-und-landschaftsbauer/${item.city}`}>{item.city.toLocaleUpperCase()}</Link></div>
                                </>
                            )
                        })
                    }
