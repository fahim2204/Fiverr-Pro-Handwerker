...
const Region = () => {
    return (
        <>
            ...
            <header className="hero-img">
                ...
                <div className="container-fluid mt-5 pt-5">
                    <div className="container">
                        <h1 className="title-text display-3">Garten und Landschaftsbauer -
                            Stadt wählen</h1>
                        <h2 className="txt2">Garten und Landschaftsbauer Experte -
            ...
            <div className="container">
                <h3 className="fs-2 py-4 mt-3 fw-bold">Finden Sie Ihren Garten und Landschaftsbauer Service in Ihrer Stadt</h3>
                <div id="alphaLink" className="d-flex bg-secondary text-white px-2 mb-4 align-items-center flex-wrap">
                    {
                        alpha.map(letter => {
                            return (
                                <>
                                    <Link className="alpha-pagination" href={`/garten-und-landschaftsbauer#starWith_${letter}`}>{letter}</Link>

                                </>
                            )
                        })
                    }
                    <Link className="alpha-pagination" href={`/garten-und-landschaftsbauer#starWith_idn`}>IDN</Link>
                </div>
                {
                    alpha.map((letter) => {
                        return (
                            <>
                                <div id={`starWith_${letter}`}>
                                    <div className="border-top border-bottom bg-light d-flex justify-content-between align-items-center">
                                        ...
                                        <div className="px-3"><Link className="text-decoration-underline hov-cl-yellow" href={`/garten-und-landschaftsbauer#alphaLink`}>Back to list</Link></div>
                                    </div>
                                    <div className="row py-3">
                                        {
                                            CityList.filter(item => item.city.startsWith(letter)).map((item) => {
                                                return (
                                                    <>
                                                        <div className="city-box col-6 col-sm-4 col-md-3"><Link className="m-3 w-100 border-bottom" href={`/garten-und-landschaftsbauer/${item.cityLink}`}>{item.city}</Link></div>
                                                    </>
                                                )
                                            })
                                        }
                ...
                <div className="row py-3">
                    {
                        CityList.filter(item => item.city.charAt(0).match(/[^a-z]/i)).map((item) => {
                            return (
                                <>
                                    <div className="col-3 mb-3 hov-cl-yellow d-flex justify-content-center align-items-center"><Link className="m-3 w-100 border-bottom" href={`/garten-und-landschaftsbauer/${item.cityLink}`}>{item.city}</Link></div>
                                </>
                            )
                        })
                    }
